...
import org.springframework.android.support.v4.cache.disk.FileCountLimitDiskCache;
...
public class LimitedCacheImageService extends NetworkImageService {

    public LimitedCacheImageService() {
        super("LimitedCacheImageService");
    }
    ...
    protected Cache initCache() {
        ...
        File dir = new File(getBaseContext().getCacheDir(), "LimitedCacheImageServiceDiskCache");
        int limit = Math.round(AppConstants.TOTAL_NUMBER_IMAGES * 0.75f);
        FileCountLimitDiskCache<String, Bitmap> cache = 
                new FileCountLimitDiskCache<String, Bitmap>("LimitedCacheImageServiceDiskCache", dir, template, limit);
        return cache;
    }
